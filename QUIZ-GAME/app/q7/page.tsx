...
export default function Q7Page() {
  ...
  const avatar = search.get('avatar') ?? '';  // 如需给后续API用，可保留
  const choice = search.get('choice') ?? '';  // 如需给后续API用，可保留

  const [selected, setSelected] = useState<number | null>(null);

  // 进页回填：如果之前答过 q7，就自动选中
  useEffect(() => {
    const a = getAnswers();
