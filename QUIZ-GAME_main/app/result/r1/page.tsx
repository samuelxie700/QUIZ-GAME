...
const MODAL: Record<ModalKey, { title: string; body: string }> = {
  ...
  partner: { title: 'Partner', body: 'Creative Learner' },
  motto: { title: 'Motto', body: '' },
};
...
// Type guard to quietly ignore AbortError.
...
export default function PageR1() {
  ...
  const [motto, setMotto] = useState<string>(''); // dynamic motto
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const cached = typeof window !== 'undefined' ? localStorage.getItem('motto_r1') || '' : '';
        if (cached) setMotto(cached);

        const resp = await fetch(`/api/motto?t=${Date.now()}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ persona: 'City Visionary' }),
        ...
        if (!aborted) {
          setMotto(finalText);
          if (typeof window !== 'undefined') localStorage.setItem('motto_r1', finalText);
        }
      } catch (err: unknown) {
        if (isAbortError(err)) return; // ignore abort during unmount
        // eslint-disable-next-line no-console
        console.error('[r1] motto api failed:', err);
        if (!aborted) setMotto('Explore learn and thrive');
      } finally {
    ...
    <main className="min-h-screen flex items-center justify-center bg-neutral-200">
      ...
      >
        ...
        >
          ...
          <div
            className={`${dongle700.className} absolute left-[12px] right-[12px] top-[40px] text-center`}
            style={{ fontSize: 18, lineHeight: '22px', color: '#4D688C', wordBreak: 'break-word' }}
          >
            {loading ? 'Generating...' : (motto || 'Explore learn and thrive')}
          </div>
        </button>
