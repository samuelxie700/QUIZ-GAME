...
function Sprite({ it, mode }: { it: Item; mode: 'static' | 'exitUp' | 'fade' | 'drop' }) {
  ...
  const style: React.CSSProperties =
    mode === 'exitUp'
      ...
      : mode === 'fade'
      ? { ...base, animation: `fadeOutSlow ${EXIT_FADE_MS}ms ease forwards` } // Trove 淡出
      : mode === 'drop'
...
export default function Page() {
  ...
  // 初始：showIdle = true（展示 Group52 静态）
  ...
  const goUnder = () => {
    hideIdle();
    setPicked('Under'); setExiting(null); setView('Under');
  };

  const goWell = () => {
    hideIdle();
    setPicked('Well'); setExiting(null); setView('Well');
  };
  ...
  const goTrove = () => {
    ...
    if (view === 'Under' || view === 'Well') {
      setExiting(view);                        // 先让当前组向上退出
      setTimeout(() => { setView('Trove'); setExiting(null); }, EXIT_UP_MS);
    } else {
      setView('Trove');                        // 从默认/其它空态直接进入
    }
  };
  ...
  return (
    <main style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#fff' }}>
      ...
      >
        ...
        {/* Over：Group52 从上落入（从默认态切过来也会执行 drop） */}
